...
class VAE(nn.Module):
    ...
    def __init__(self, checkpoint_path=os.path.join(smiles_gpt_path, "checkpoints/benchmark-5m")):
        ...
        self.conv_2 = nn.Conv1d(in_channels=9, out_channels=11, kernel_size=9)
        self.conv_3 = nn.Conv1d(in_channels=11, out_channels=11, kernel_size=11)

        # Mismatch in features will cause a runtime error
        self.fc_0 = nn.Linear(in_features=128, out_features=435)
        ...
        self.fc_to_gpt_embed = nn.Linear(in_features=292, out_features=51200)
        self.smiles_gpt = GPT2LMHeadModel.from_pretrained(checkpoint_path)
        self.final_projection = nn.Linear(512, len(CHARSET))
    ...
    def decode(self, z):
        # Reshaping is incorrect and doesn't use the GPT model correctly
        z_reshaped = z.view(z.size(0), 100, 512)
        # The output from this layer won't match the input for loss calculation
        output = self.final_projection(z_reshaped)
        return output
