...
class VAE(nn.Module):
    ...
    def __init__(self, checkpoint_path=os.path.join(smiles_gpt_path, "checkpoints/benchmark-5m")):
        ...
        self.fc_3 = nn.Linear(in_features=292, out_features=512 * 100)
        self.fc_4 = nn.Linear(in_features=512, out_features=len(CHARSET))  # Added fc_4 layer
    ...
    def decode(self, z, temperature=1.0, top_k=10):
        ...
        embedding_size = 512  # Calculate embedding size dynamically
...
def train(train_data=os.path.join(),
          val_data=os.path.join(''),
          path_checkpoint=r'',
        ...
        try:
            ...
            if resume_from_checkpoint:
                ...
                if checkpoint_files:
                    ...
                    checkpoint = torch.load(last_checkpoint_path, map_location=device, weights_only=True) # weights_only=True
            ...
            best_val_epoch = -1  # Added for the best model tracking
